...
class EncoderCNN(nn.Module):
    def __init__(self, encoded_image_size, cnn, device):
        ...
        self.faster_rcnn_threshold = 0.6
        self.img_path = "dataset/Flickr8k_Dataset/"

        #faster rcnn crops
        self.linear = nn.Linear(2048, 1024)
        modules = list(resnet.children())[:-1]      # delete the last fc layer.
        self.object_crop_encoder = nn.Sequential(*modules)

        if torch.cuda.is_available():
            self.object_crop_encoder.cuda()
    ...
    def encoder_object_crop(self, img):
        ...
        with torch.no_grad():
            features = self.object_crop_encoder(img)
...
class DecoderRNNWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, captions, lengths, device):
        ...
        for t in range(max_length):
            ...
            predictions[:batch_size_t, t, :] = self.fc(self.dropout(hidden))  # (batch_size_t, vocab_size)
